...
For this reason it is needed to first define a JS function in the dedicated _[annotations.js](https://github.com/Tars4815/ponti/blob/main/js/annotations.js)_ file.
...
First, it is needed to define a custom form in the main GUI, allowing users to choose within possible annotation types to be added and then to define annotation parameters (title, position, camera settings, description). A new button for this purpose is created in [index.php](https://github.com/Tars4815/ponti/blob/main/index.php).
...
Let's now define the appearance and the style of the Edit (and Delete too) buttons in [style.css](https://github.com/Tars4815/ponti/blob/main/css/style.css):
...
Another useful operation for annotations is the UPDATE one. In order to do so, first an *Edit button* (as well as a *Delete button* that will be explained in the next DELETE annotation section) has been added in the Potree annotation constructor in [potree.js](https://github.com/Tars4815/ponti/blob/main/libs/potree/potree.js) when the structure for the *annotation-description* is defined:
...
The [update_annotation.php](https://github.com/Tars4815/ponti/blob/main/database/update_annotation.php) file deals with the updated annotation parameters and, identifying the record to be modified through the annotation id, save the new values in the database.
