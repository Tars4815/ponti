...
// Function to get query parameters from a given URL
function getQueryParam(param, url) {
  let urlParams = new URL(url).searchParams;
  return urlParams.get(param);
}

// Get the filepath parameter from the current script URL
let scriptUrl = import.meta.url;
let filepath = getQueryParam("filepath", scriptUrl);

console.log("Retrieved filepath: " + filepath);
...
if (filepath) {
  let filepathfull = "../pointclouds/"+ filepath + "/metadata.json";
  ...
  console.error("No filepath specified for point cloud data.");
}
