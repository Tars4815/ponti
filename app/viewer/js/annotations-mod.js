// Annotations Module for Potree Viewer
// This module handles the creation, editing, and deletion of annotations in the Potree viewer.
// It allows users to add comments and defects with specific attributes, save them to a database, and manage them in the scene.
// Store the inspection ID from the URL parameters
...
/**
 * Set styles for the annotation based on its type.
 * @param {object} annotation - The annotation object to which styles will be applied.
 * @param {string} annotationType - The type of the annotation (e.g., "comments", "defect", "structural element").
 * @throws {Error} Will throw an error if the annotation type is unknown.
 */
...
function saveAnnotation(
  ...
  console.log("inspectionId value before sending defect:", inspectionId);
  ...
  $.ajax({
    ...
    success: function (id) {
      ...
      console.log(`${annotationType} annotation created with ID: ${id}, inspection ID: ${inspectionId}`);
      console.log("Inspection ID:", inspectionId);
    },
    ...
    success: function (existingDefects) {
      ...
      existingDefects.forEach((annotation) => {
        createAnnotation(
          ...
          [],
          [],
          "<b>Type: </b>" + annotation.type + "</br><b>Severity level: </b>" + annotation.severity + "</br><b>Comments: </b>" + annotation.note,
          "defect",
          annotation.type,
          annotation.severity
        );
...
document.addEventListener("DOMContentLoaded", function () {
  $("#pickPointButton").click(function () {
    ...
    measurement.addEventListener("marker_dropped", (e) => {
      ...
      // Get scalar field/structural el. code for clicked point
